...
test_latest_uses_published_version_file() {
  ...
  [[ "$(<"$TEST_ROOT/prefix/share/selfishell/current/VERSION")" == 0.2.2 ]] ||
...
test_cli_update_and_offline_rollback() {
  ...
  "$TEST_ROOT/prefix/bin/selfishell" update --cli-only --version 0.2.2 --yes >/dev/null
  assert_symlink_to 'releases/0.2.2' "$TEST_ROOT/prefix/share/selfishell/current"
  ...
  assert_symlink_to "releases/$version" "$TEST_ROOT/prefix/share/selfishell/current"
  assert_symlink_to 'releases/0.2.2' "$TEST_ROOT/prefix/share/selfishell/previous"
}
...
test_default_update_skips_missing_configuration_and_updates_cli() {
  ...
  output="$("$TEST_ROOT/prefix/bin/selfishell" update --version 0.2.2 --yes)"
...
test_update_dry_run_preserves_active_release() {
  ...
  output="$("$TEST_ROOT/prefix/bin/selfishell" update --version 0.2.2 --dry-run)"
  [[ "$output" == *'Would update Selfishell CLI to 0.2.2.'* ]] ||
    fail "Update dry-run did not preview the CLI release"
  assert_symlink_to "releases/$version" "$TEST_ROOT/prefix/share/selfishell/current"
  [[ ! -e "$TEST_ROOT/prefix/share/selfishell/releases/0.2.2" ]] ||
...
test_bootstrap_upgrade_retains_rollback_and_prunes_inactive_release() {
  ...
  run_bootstrap --version 0.2.2 >/dev/null
