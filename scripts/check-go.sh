...
export GOOS GOARCH
formatting="$(gofmt -l cmd internal tests/migration)"
...
SELFISHELL_TEST_CLI="$ROOT_DIR/.build/targets/$GOOS-$GOARCH/bin/selfishell" go test ./tests/migration -run '^TestFoundation$' -count=1
